	...
	return F(
		...
		func(bui B) {
			if page.GetTitle() != `` {
				...
				bui.E(`title`, nil, `btcse.org`)
			}

			bui.E(
				`link`,
				AP(`rel`, `alternate`, `type`, `application/atom+xml`, `title`, `BTCSE's Posts (Atom)`, `href`, `/feed.xml`),
			)
			bui.E(
				`link`,
				AP(`rel`, `alternate`, `type`, `application/rss+xml`, `title`, `BTCSE's Posts (RSS)`, `href`, `/feed_rss.xml`),
			)
			bui.E(`meta`, AP(`name`, `author`, `content`, `btcse.org`))
			if page.GetTitle() != `` {
			...
			bui.E(`meta`, AP(`property`, `og:site_name`, `content`, `btcse.org`))
...
func Header[A Ipage](page A) (out x.Elem) {
	return
...
var FeedLinks = E(`p`, AP(`class`, `inl-flex row-sta-sta gap-hor-0x5`),
	...
	LinkExt(`https://feedly.com/i/subscription/feed/https://static.btcse.org/feed.xml`, `Feedly`),
)
